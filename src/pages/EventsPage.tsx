      ...
      <div id="events-section" className="px-6 md:px-12 lg:px-24 py-8">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-up">
            {events.map((event) => (
              <div key={event.id} className="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all hover:scale-105 w-[300px]">
                
                {/* Image Box with White Border */}
                <div className="w-full h-[200px] bg-white border-4 border-white flex items-center justify-center overflow-hidden">
                  <img src={event.thumbnail_url} alt={event.title} className="w-full h-full object-cover" />
                </div>

                {/* Title */}
                <div className="text-gray-900 text-center py-2 font-semibold text-lg">
                  {event.title}
                </div>
